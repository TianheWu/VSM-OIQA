...
def split_dataset_Fang2022(dis_path, split_seed=20):
...
def split_dataset_ke2020(dis_path, split_seed=20):
    np.random.seed(split_seed)
    dis_data = {}
    name_list = []
    for root, ds, fs in os.walk(dis_path):
    ...
    l = len(name_list)
    train_name_idx = name_list[:round(l * 0.8)]
    val_name_idx = name_list[round(l * 0.8):]
