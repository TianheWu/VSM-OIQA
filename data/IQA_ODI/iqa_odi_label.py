...
class IQAODI(torch.utils.data.Dataset):
    ...
    def select_random_viewport(self, x):
        """ Random choose the viewport
        Args:
            x ([numpy]): the ERP images
        
        Return:
            x ([list]): the list including viewports
        """
        ret = []
        for i in range(self.viewport_num):
